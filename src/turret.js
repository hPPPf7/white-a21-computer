...
export function createTurret(renderer){
 ...
 function wire(n,a,b,points,r=.018,m=rubber){const old=part;part='wiring';const path=new THREE.CatmullRomCurve3(points.map(p=>new THREE.Vector3(...p)));add(new THREE.TubeGeometry(path,Math.max(24,points.length*5),r,6,false),n,[0,0,0],m);links.push({name:n,from:a.name,to:b.name,start:points[0],end:points.at(-1),a,b});part=old;}
 ...
 plate('Motherboard tray',3.80,3.67,[0,2.62,-.82],[[1.30,.58,.39,.8],[1.3,-.67,.39,.59],[-.8,1.59,.60,.24]]);
