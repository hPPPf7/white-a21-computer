...
Chrome 同視角量測：WHITE A21 每幀繪製呼叫 953 → 258、三角形 484,454 → 218,678；HYPERION 520 → 158、228,948 → 148,756。以裝置像素比例 2 比較，1440 × 1000 桌面渲染像素量減少約 54%，390 × 844 手機減少 75%。這些是繪圖工作量，並非 FPS 加速倍數；代價是高 DPI 細節及陰影精度降低。

進一步按父節點、配件、材質與陰影設定合併不透明靜態幾何，保留原有三角形、風扇動畫及接線端點。相較上一版，繪製呼叫由 650 → 258、464 → 158。被合併的原始物件保持隱藏，配件隔離不會重複繪製。滑鼠命中測試只檢查可見幾何、最多每 75ms 一次，網孔透明度資料一次讀取後重用。

## 機殼製造細節

A21 增加前網折邊、面板卡扣、頂部濾網包邊與拉片、框架折邊、玻璃隔離墊片、螺絲凹槽、腳座固定板及 USB／音源孔金屬邊。後方風扇、電源及 PCI 區域使用貫穿開口。
